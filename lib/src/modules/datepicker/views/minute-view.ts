...
export class CalendarRangeMinuteService extends CalendarRangeService {
  ...
  public configureItem(item:CalendarItem, _baseDate:Date):void {
    item.humanReadable = new DateParser(this.service.localeValues.formats.time, this.service.localeValues).format(
      item.date
    );
    ...
    <table class="ui celled center aligned unstackable table three column minute">
      ...
      <tbody>
        <tr *ngFor="let group of ranges.current.groupedItems">
          <td class="link" *ngFor="let item of group" [calendarItem]="item" (click)="setDate(item)">
            {{ item.humanReadable }}
          </td>
        </tr>
      </tbody>
